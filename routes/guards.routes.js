...
const Guards = require('../models/guards.models');

const router = express.Router();

router.get('/', async (req, res) => {
  const guards = await Guards.find({}).sort('-name');
  if (!guards.length) {
    res.json('No guard was found!');
  } else res.json(guards);
});
...
router.delete('/:id', async (req, res) => {
  const data = await Guards.findByIdAndDelete(req.params.id);
  res.json({
